...
def inv_linear_spectrogram(linear_spectrogram, hparams):
    ...
    S = _db_to_amp(D + hparams.ref_level_db, hparams)  # Convert back to linear
...
def inv_spectrogram_tensorflow(spectrogram, hparams):
    ...
    S = _db_to_amp_tensorflow(D + hparams.ref_level_db, hparams)
    return _griffin_lim_tensorflow(tf.pow(S, hparams.power), hparams)


def inv_spectrogram_tensorflow_batch(spectrogram, hparams):
    '''Builds computational graph to convert spectrogram to waveform using TensorFlow.
    Unlike inv_spectrogram, this does NOT invert the preemphasis. The caller should call
    inv_preemphasis on the output after running the graph.
    '''
    ...
    S = _db_to_amp_tensorflow(D + hparams.ref_level_db, hparams)
    return _griffin_lim_tensorflow_batch(tf.pow(S, hparams.power), hparams)
...
def _griffin_lim_tensorflow(S, hparams):
    ...
    with tf.variable_scope('griffinlim'):
        ...
        y = tf.contrib.signal.inverse_stft(
            S_complex, hparams.win_size, get_hop_size(hparams), hparams.n_fft)
        for i in range(hparams.griffin_lim_iters):
            est = tf.contrib.signal.stft(
                y, hparams.win_size, get_hop_size(hparams), hparams.n_fft)
            angles = est / tf.cast(tf.maximum(1e-8, tf.abs(est)), tf.complex64)
            y = tf.contrib.signal.inverse_stft(
                S_complex * angles, hparams.win_size, get_hop_size(hparams), hparams.n_fft)
    return tf.squeeze(y, 0)


def _griffin_lim_tensorflow_batch(S, hparams):
    '''TensorFlow implementation of Griffin-Lim
    Based on https://github.com/Kyubyong/tensorflow-exercises/blob/master/Audio_Processing.ipynb
    '''
    with tf.variable_scope('griffinlim_batch'):
        S_complex = tf.identity(tf.cast(S, dtype=tf.complex64))
        y = tf.contrib.signal.inverse_stft(
            S_complex, hparams.win_size, get_hop_size(hparams), hparams.n_fft)
        for i in range(hparams.griffin_lim_iters):
...
def _amp_to_db(x, hparams):
    ...
    if hparams.waveglow_todb:
        return np.log(np.maximum(min_level, x))
    else:
        return 20 * np.log10(np.maximum(min_level, x))


def _db_to_amp(x, hparams):
    if hparams.waveglow_todb:
        return np.exp(x)
    else:
...
def _db_to_amp_tensorflow(x, hparams):
    if hparams.waveglow_todb:
        return tf.exp(x)
    else:
