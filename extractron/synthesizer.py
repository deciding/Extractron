...
from tacotron.utils.text import seq_to_cnen_mask
from tacotron.utils.text import phoneme_str_to_seq
...
class Synthesizer:
    ...
    def synthesize(self, texts, basenames, out_dir, log_dir, mel_filenames, spknm=''):
        wavs_or_mel, sr_or_align, _ = self.synthesize_detail(texts, basenames, out_dir, log_dir, mel_filenames, spknm)
        return wavs_or_mel, sr_or_align
    ...
    def eval(self, text):
        ...
        linear_wavs = self.session.run(
            self.linear_wav_outputs, feed_dict=feed_dict)
        wav = audio.inv_preemphasis(linear_wavs, hparams.preemphasis)
        #out = io.BytesIO()
    ...
    def _prepare_lang_masks(self, lang_masks):
        max_len = max([len(x) for x in lang_masks])
        return np.stack([self._pad_lang_mask(x, max_len) for x in lang_masks]), max_len

    def _pad_input(self, x, length):
        return np.pad(x, (0, length - x.shape[0]), mode='constant', constant_values=self._pad)

    def _pad_lang_mask(self, x, length):
        return np.pad(x, (0, length - x.shape[0]), mode='edge')
    ...
    def _get_output_lengths(self, stop_tokens):
        ...
        output_lengths = [row.index(1)
                          for row in np.round(stop_tokens).tolist()]
